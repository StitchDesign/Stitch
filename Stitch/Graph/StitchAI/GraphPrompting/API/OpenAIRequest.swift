...
extension StitchAIManager {
    ...
    func makePlanningRequest(userPrompt: String) async throws -> String {
        let systemPrompt = """

        You are the reasoning module for Stitch. 
        
        # Fundamental Principles

        Stitch uses a visual programming language and is similar to Meta's Origami Studio.

        Your primary purpose is to create a response that generates a plan for creating a graph
        based on the users input. Your plan will then be used by another AI to generate graph
        actions following a user's prompt. The user may request logic and/or UI for their prototype.
        ...
        - Do not ouptut JSON; just a bullet point, step-sequence that another AI can use to generate the graph from


        Below is the schema that Stitch uses under the hood to generate graphs. 
        ...
        # Value Examples
        Below is a schema illustrating various value types and the types of values they take.
        ```
        {
          "valueTypes" : [
            {
        ...
        do {
            ...
            log("Failed to encode planning request body: \(error)", .logToServer)
            throw StitchAIStreamingError.other(error)
        }
        ...
        if let httpResponse = response as? HTTPURLResponse,
           !(200...299).contains(httpResponse.statusCode) {
            log("Planning request failed with status code: \(httpResponse.statusCode)", .logToServer)
            throw StitchAIStreamingError.other(
                NSError(domain: "OpenAIHTTPError", code: httpResponse.statusCode)
            )
        }
        
        let json = try JSON(data: data)
        let content = json["choices"][0]["message"]["content"].stringValue.trimmingCharacters(in: .whitespacesAndNewlines)
