...
extension StitchDocumentViewModel {
    ...
    // TODO: do more decoding logic on the `LLMStepAction`-side; e.g. `LLMStepAction.nodeName` should be type `PatchOrLayer` rather than `String?`
    @MainActor
    ...
    var parseLLMNodeTitle: (String, NodeKind)? {
        ...
        var getNodeKind: NodeKind? {
            ...
            let specialCases: [String: NodeKind] = [
                ...
                "circle shape": .patch(.circleShape),
                "oval shape": .patch(.ovalShape),
                "split text": .patch(.splitText),
                "device time": .patch(.deviceTime),
                "device info": .patch(.deviceInfo),
