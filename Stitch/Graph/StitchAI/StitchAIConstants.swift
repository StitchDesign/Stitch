...
// MARK: - Main Structure
struct VisualProgrammingActions: Codable {
    let steps: [ProgrammingAction]
}

// MARK: - Actions
enum ProgrammingAction: Codable {
    case addNode(AddNodeAction)
    case connectNodes(ConnectNodesAction)
    case changeNodeType(ChangeNodeTypeAction)
    case setInput(SetInputAction)
    case addLayerInput(AddLayerInputAction)
    ...
    init(from decoder: Decoder) throws {
        ...
        let stepType = try container.decode(String.self, forKey: .stepType)
        
        switch stepType {
        case "add_node":
            self = .addNode(try AddNodeAction(from: decoder))
        case "connect_nodes":
            self = .connectNodes(try ConnectNodesAction(from: decoder))
        case "change_node_type":
            self = .changeNodeType(try ChangeNodeTypeAction(from: decoder))
        case "set_input":
            self = .setInput(try SetInputAction(from: decoder))
        case "add_layer_input":
            self = .addLayerInput(try AddLayerInputAction(from: decoder))
        default:
            throw DecodingError.dataCorruptedError(forKey: .stepType, in: container, debugDescription: "Unknown step type")
        }
    }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        switch self {
        case .addNode(let action):
            try container.encode("add_node", forKey: .stepType)
            try action.encode(to: encoder)
        case .connectNodes(let action):
            try container.encode("connect_nodes", forKey: .stepType)
            try action.encode(to: encoder)
        case .changeNodeType(let action):
            try container.encode("change_node_type", forKey: .stepType)
            try action.encode(to: encoder)
        case .setInput(let action):
            try container.encode("set_input", forKey: .stepType)
            try action.encode(to: encoder)
        case .addLayerInput(let action):
            try container.encode("add_layer_input", forKey: .stepType)
            try action.encode(to: encoder)
        }
    }
}

// MARK: - Action Structures
struct AddNodeAction: Codable {
    let nodeName: String
    let nodeId: UUID
...
struct ConnectNodesAction: Codable {
    let fromNodeId: UUID
    let toNodeId: UUID
    let port: Port
...
struct ChangeNodeTypeAction: Codable {
    let nodeId: UUID
...
struct AddLayerInputAction: Codable {
    let nodeId: UUID
...
enum LayerPorts: String, Codable {
    case text = "Text"
    case scale = "Scale"
    case shape = "Shape"
    case image = "Image"
...
enum Port: Codable {
    case integer(Int)
    case layerPort(LayerPorts)
    
    init(from decoder: Decoder) throws {
        let container = try decoder.singleValueContainer()
        if let intValue = try? container.decode(Int.self) {
            self = .integer(intValue)
        } else if let layerPort = try? container.decode(LayerPorts.self) {
            self = .layerPort(layerPort)
        } else {
            throw DecodingError.dataCorruptedError(in: container, debugDescription: "Port must be either an integer or LayerPorts value")
        }
    }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        case .layerPort(let layerPort):
            try container.encode(layerPort)
        }
    }
}
...
enum InputValue: Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        if let doubleValue = try? container.decode(Double.self) {
            ...
            throw DecodingError.dataCorruptedError(in: container, debugDescription: "Value must be a number, string, or boolean")
        }
    }
