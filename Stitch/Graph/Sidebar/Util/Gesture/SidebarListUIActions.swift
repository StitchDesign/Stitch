...
extension ProjectSidebarObservable {
    ...
    func moveSidebarListItemIntoGroup(_ item: ItemViewModel,
        ...
        for otherSelection in ([item.id] + otherSelections) {
            guard let otherItem = items.first(where: { $0.id == otherSelection }) else {
                fatalErrorIfDebug("Could not retrieve item")
                continue
            }
            
            self.items.remove(otherSelection)
            
            proposedGroup.children?.append(otherItem)
