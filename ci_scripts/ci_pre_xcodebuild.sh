...
printf '{
    ...
    "SENTRY_DSN": "%s"
}' "$SUPABASE_URL" "$SUPABASE_ANON_KEY" "$SUPABASE_USER_PROMPT_TABLE_NAME" "$SUPABASE_INFERENCE_CALL_RESULT_TABLE_NAME" "$OPEN_AI_API_KEY" "$OPEN_AI_MODEL_GRAPH_CREATION" "$OPEN_AI_MODEL_JS_NODE" "$OPEN_AI_MODEL_GRAPH_DESCRIPTION" "$SENTRY_DSN" > secrets.json
